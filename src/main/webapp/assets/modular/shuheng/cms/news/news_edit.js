...
layui.use(['layer', 'form', 'admin', 'laydate', 'ax','upload'], function () {
    ...
    var ajax = new $ax(Feng.ctxPath + "/news/getNewsInfo?newsId=" + Feng.getUrlParam("newsId"));
    var result = ajax.start();
    //回显图片
    if(result.data.newsImg!=""){
        $("#imgShow").attr("src",Feng.ctxPath + result.data.newsImg );
    }
    //回显视频
    if(result.data.newsVideoPath!=""){
        $("#newsVideoSpan").text(result.data.newsVideoName);
    }
    ue.ready(function() {
        ue.setContent(result.data.newsContent, false);
    });
    form.val('newsForm', result.data);

    //根据不同模型控制显示隐藏项
    var newsModel = result.data.newsModel
    if(newsModel=="news"){
        $("#modelVideo").hide()
        $("#modelDownload").hide()
    }
    ...
    if(newsModel=="download"){
        $("#modelVideo").hide()
        $("#newsFileSpan").text(result.data.newsFileName);
    }
